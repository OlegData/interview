...
пше Для загрузки больших файлов в FastAPI используйте UploadFile, а не чтение всего файла в память. UploadFile сохраняет данные во временный файл и поддерживает асинхронное чтение по частям. Обрабатывайте файл чанками, чтобы не перегружать память. Также настройте лимиты на уровне сервера (reverse proxy/ASGI) и валидируйте размер и тип файла перед обработкой.
